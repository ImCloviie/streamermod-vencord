...
import { ColorPicker, Forms, UserStore } from "@webpack/common";
...
const colorPresets = [
    "#3ba55d", "#ed4245", "#fee75c", "#5865f2", "#eb459e",
    "#ffffff", "#99aab5", "#2c2f33", "#23272a", "#000000"
];
...
const settings = definePluginSettings({
    ...
    colorOn: {
        ...
        default: "#3ba55d",
        component: () => {
            const { colorOn } = settings.use(["colorOn"]);
            return (
                <div style={{ marginBottom: "20px" }}>
                    <Forms.FormTitle tag="h3">Цвет включенного состояния</Forms.FormTitle>
                    <Forms.FormText style={{ marginBottom: "8px" }}>Цвет кнопки, когда защита от захвата ВКЛЮЧЕНА.</Forms.FormText>
                    <ColorPicker
                        color={parseInt(colorOn.replace("#", ""), 16)}
                        onChange={(c: number) => {
                            settings.store.colorOn = "#" + c.toString(16).padStart(6, "0");
                            updateButtonState();
                        }}
    ...
    colorOff: {
        ...
        default: "#ed4245",
        component: () => {
            const { colorOff } = settings.use(["colorOff"]);
            return (
                <div style={{ marginBottom: "20px" }}>
                    <Forms.FormTitle tag="h3">Цвет выключенного состояния</Forms.FormTitle>
                    <Forms.FormText style={{ marginBottom: "8px" }}>Цвет кнопки, когда защита от захвата ВЫКЛЮЧЕНА.</Forms.FormText>
                    <ColorPicker
                        color={parseInt(colorOff.replace("#", ""), 16)}
                        onChange={(c: number) => {
                            settings.store.colorOff = "#" + c.toString(16).padStart(6, "0");
                            updateButtonState();
                        }}
                        suggestedColors={colorPresets}
                    />
...
export default definePlugin({
    name: "StreamerMod",
    description: "Скрывает Discord от захвата экрана (демка, OBS, запись). Ты видишь Discord, но на записи его нет.",
    tags: ["Privacy", "Utility"],
    authors: [{
        name: "imcloviie",
        id: 929002684000899082n
    }],

    settingsAboutComponent: () => (
        <div style={{ marginTop: "16px" }}>
            <Forms.FormTitle tag="h3">Ссылки</Forms.FormTitle>
            <Link href="https://github.com/ImCloviie">
                GitHub автора (imcloviie)
            </Link>
        </div>
    ),
